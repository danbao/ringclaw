...
| `/cc 解释一下这段代码`       | 通过别名发送                               |
| `@claude 解释一下`           | @ 前缀 — 等同于 `/claude 解释一下`         |
...
`@` 前缀是 `/` 的替代写法，用于指定 Agent：

```
@claude 解释一下这段代码       # 等同于 /claude 解释一下这段代码
@cc @cx review this function  # 同时广播给 Claude 和 Codex，并行处理
```

广播时，每个 Agent 的回复以 `[agent-name]` 前缀分别发送。
...
之后 `/gpt 你好` 或 `@ai 你好` 都会路由到 Claude。如果自定义别名与内置别名或其他 Agent 冲突，RingClaw 启动时会发出警告。
